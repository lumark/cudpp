...
int CheckResults_basic(const unsigned kInputSize,
                       ...
                       const unsigned *query_keys, const unsigned *query_vals) {
  int errors = 0;
  for (unsigned i = 0; i < kInputSize; ++i) {
    // @TODO is this right?
    unsigned actual_value = CUDPP_HASH_KEY_NOT_FOUND;
    std::map<unsigned, unsigned>::const_iterator it = pairs.find(query_keys[i]);
    if (it != pairs.end()) {
...
#ifdef _DEBUG
      printf(
          "\t\t\tError for key %10u: Actual value is "
...
int CheckResults_compacting(const unsigned kInputSize,
...
#ifdef _DEBUG
          fprintf(stderr, "\t\t\t\t!!! Key %10u had two IDs: %10u %10u\n",
                  query_keys[j], key_to_id_map[query_keys[j]], query_vals[j]);
#endif
          errors++;
        }
      }
...
#ifdef _DEBUG
          fprintf(stderr, "\t\t\t\t!!! ID %10u had two keys: %10u %10u\n",
                  query_vals[j], id_to_key_map[query_vals[j]], query_keys[j]);
#endif
          errors++;
        }
      }
  ...
  if (key_to_id_map.size() != id_to_key_map.size()) {
    fprintf(stderr,
            "\t\t\t\t!!! Number of unique IDs doesn't match the "
...
int CheckResults_multivalue(
    const unsigned kInputSize,
    const std::map<unsigned, std::vector<unsigned> > &pairs,
    const unsigned *sorted_values, const unsigned *query_keys,
    ...
    if (itr != pairs.end()) {
      ...
      if (query_vals_multivalue[i].y != itr->second.size()) {
        fprintf(stderr,
                ...
                "values instead of %u.\n",
                query_keys[i], query_vals_multivalue[i].y,
      ...
      for (unsigned j = 0; j < query_vals_multivalue[i].y; ++j) {
        if (hash_table_values[j] != itr->second[j]) {
          fprintf(stderr, "\t\t\t\t!!! Values didn't match: %10u != %10u\n",
                  hash_table_values[j], itr->second[j]);
          errors++;
        }
      }
    } else {
...
//// looks like we can use it in the device code

//__global__ void CheckHashRetrieve(CUDPPHandle &pHandle,
//                                  unsigned int *d_test_keys,
//                                  unsigned int *query_vals) {
//  cudppHashRetrieve(hash_table_handle, d_test_keys, d_test_vals_multivalue,
//                    kInputSize);
//}

// void testHashTableInDevice() {}
 ...
 */
int testHashTable(CUDPPHandle theCudpp, CUDPPHashTableType htt,
                  unsigned int kMaxIterations, unsigned int kInputSize,
                  unsigned int *number_pool, unsigned int pool_size,
                  unsigned int *input_vals, unsigned int *input_keys,
                  unsigned int *d_test_vals, uint2 *d_test_vals_multivalue,
                  unsigned int *d_test_keys, unsigned int *query_vals,
                  uint2 *query_vals_multivalue, unsigned int *query_keys,
                  unsigned int skipEveryNTests, bool skipHighMultiplicities,
  ...
  for (unsigned iteration = 0; iteration < kMaxIterations; ++iteration) {
    ...
    for (unsigned multiplicity = 1; multiplicity <= multiplicity_max;
         multiplicity *= 2) {
      ...
      for (unsigned i = 0; i < kNumSpaceUsagesToTest; ++i) {
        ...
        result = cudppHashInsert(hash_table_handle, d_test_keys, d_test_vals,
                                 kInputSize);
        cudaThreadSynchronize();
        timer.stop();
        if (result != CUDPP_SUCCESS) {
          fprintf(stderr,
                  "Error in cudppHashInsert call in"
                  "testHashTable\n");
        }
        ...
        if (htt == CUDPP_MULTIVALUE_HASH_TABLE) {
          ...
          if (cudppMultivalueHashGetAllValues(hash_table_handle,
                                              &d_all_values) != CUDPP_SUCCESS) {
            fprintf(stderr,
                    "Error: "
                    "cudppMultivalueHashGetAllValues()\n");
          }

          CUDA_SAFE_CALL(cudaMemcpy(sorted_values, d_all_values,
        ...
        for (unsigned failure = 0; failure <= failure_trials; ++failure) {
          ...
          CUDA_SAFE_CALL(cudaMemcpy(d_test_keys, query_keys,
                                    ...
                                    cudaMemcpyHostToDevice));
          CUDA_SAFE_CALL(
              cudaMemset(d_test_vals, 0, sizeof(unsigned) * kInputSize));
          ...
          if (result != CUDPP_SUCCESS) {
            fprintf(stderr,
                    "Error in cudppHashRetrieve call in"
                    "testHashTable\n");
          }
          printf(
              "\tHash table retrieve with %3u%% chance of "
              "failed queries: %f ms\n",
              failure * failure_trials, timer.getTime());
          /// --------------------------------------------------------
          ...
          switch (htt) {
            case CUDPP_BASIC_HASH_TABLE:
              CUDA_SAFE_CALL(cudaMemcpy(query_vals, d_test_vals,
                                        sizeof(unsigned) * kInputSize,
                                        cudaMemcpyDeviceToHost));
              errors += CheckResults_basic(kInputSize, pairs_basic, query_keys,
                                           query_vals);
              break;
            case CUDPP_COMPACTING_HASH_TABLE:
              CUDA_SAFE_CALL(cudaMemcpy(query_vals, d_test_vals,
                                        sizeof(unsigned) * kInputSize,
                                        cudaMemcpyDeviceToHost));
              errors += CheckResults_compacting(kInputSize, pairs_compacting,
                                                query_keys, query_vals);
              break;
            case CUDPP_MULTIVALUE_HASH_TABLE:
              CUDA_SAFE_CALL(cudaMemcpy(
                  query_vals_multivalue, d_test_vals_multivalue,
                  sizeof(uint2) * kInputSize, cudaMemcpyDeviceToHost));
              errors += CheckResults_multivalue(kInputSize, pairs_multivalue,
                                                sorted_values, query_keys,
        ...
        if (result != CUDPP_SUCCESS) {
          fprintf(stderr,
                  "Error in cudppDestroyHashTable call in"
                  "testHashTable\n");
        }
...
int main(int argc, const char **argv) {
  ...
  if (!quiet && cudaGetDeviceProperties(&prop, dev) == 0) {
    ...
    printf("%s; global mem: %uB; compute v%d.%d; clock: %d kHz\n", prop.name,
           (unsigned int)prop.totalGlobalMem, (int)prop.major, (int)prop.minor,
           (int)prop.clockRate);
  }

  if (prop.major < 2) {
    fprintf(stderr,
            "ERROR: CUDPP hash tables are only supported on "
            "devices with compute\n  capability 2.0 or greater; "
            "exiting.\n");
    exit(1);
  }

  std::cout<<"pass device checking"<<std::endl;
...
//  bool runAll = checkCommandLineFlag(argc, argv, "all");
  ...
  CUDA_SAFE_CALL(
      cudaMalloc((void **)&d_test_keys, sizeof(unsigned) * kInputSize));
  CUDA_SAFE_CALL(
      cudaMalloc((void **)&d_test_vals, sizeof(unsigned) * kInputSize));
  CUDA_SAFE_CALL(
      cudaMalloc((void **)&d_test_vals_multivalue, sizeof(uint2) * kInputSize));

  // create the cudpp handel
  CUDPPHandle theCudpp;
  CUDPPResult result = cudppCreate(&theCudpp);
  if (result != CUDPP_SUCCESS) {
    if (!quiet) fprintf(stderr, "Error initializing CUDPP Library.\n");
    retval = 1;
    return retval;
  }
  ...
  for (CUDPPHashTableType htt = CUDPP_BASIC_HASH_TABLE;
    ...
    if (runAll || ((htt == CUDPP_BASIC_HASH_TABLE) && runBasicHash) ||
        ((htt == CUDPP_COMPACTING_HASH_TABLE) && runCompactingHash) ||
        ((htt == CUDPP_MULTIVALUE_HASH_TABLE) && runMultivalueHash)) {
      std::cout<<"before testing the hash table!"<<std::endl;

      total_errors += testHashTable(
          theCudpp, htt, kMaxIterations, kInputSize, number_pool, pool_size,
          input_vals, input_keys, d_test_vals, d_test_vals_multivalue,
          d_test_keys, query_vals, query_vals_multivalue, query_keys,
          skipEveryNTests, skipHighMultiplicities, testNumber);
    }
  }
  if (total_errors == 0) {
