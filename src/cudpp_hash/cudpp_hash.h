...
class CUDPPHashTableInternal {
 public:
  ...
  CUDPPHandle getHandle() { return reinterpret_cast<CUDPPHandle>(this); }
  //! @brief Destructor for CUDPPHashTableInternal
  ~CUDPPHashTableInternal() { delete hash_table; }
};
...
CUDPP_DLL CUDPPResult cudppHashTable(CUDPPHandle cudppHandle, CUDPPHandle *plan,
                                     const CUDPPHashTableConfig *config);

CUDPP_DLL CUDPPResult
cudppDestroyHashTable(CUDPPHandle cudppHandle, CUDPPHandle plan);

CUDPP_DLL CUDPPResult cudppHashInsert(CUDPPHandle plan, const void *d_keys,
