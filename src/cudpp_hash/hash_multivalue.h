...
class MultivalueHashTable : public HashTable {
 public:
   ...
   *  @returns Whether the hash table was built successfully (true) or not
   * (false).
   ...
   */
  virtual void Retrieve(const unsigned n_queries, const unsigned *d_keys,
                        uint2 *d_location_counts);

  //! Returns the array of values, where each key's values are stored
  // contiguously in memory.
  inline __host__ __device__ const unsigned *get_all_values() const {
    return d_sorted_values_;
  }

  //! Gets the total number of values between all of the keys.
  inline __host__ __device__ unsigned get_values_size() const {
    return sorted_values_size_;
  }

  //! Gets the location and number of values each key has.
  inline __host__ __device__ const uint2 *get_index_counts() const {
    return d_index_counts_;
  }
...
void CallCheckIfUnique(const unsigned *d_sorted_keys, const size_t n,
                       unsigned *d_is_unique);

//! Calls the kernel that compacts down the unique keys.
void CallCompactKeys(const unsigned *d_keys, const unsigned *d_is_unique,
                     const unsigned *d_locations, const size_t kSize,
                     uint2 *d_index_counts, unsigned *d_compacted);

//! Calls the kernel that counts how many values each key has.
void CallCountValues(uint2 *d_index_counts, unsigned kSize,
                     unsigned num_unique);

//! Calls the kernel that crease an array containing 0 to num_unique_keys - 1.
void CallPrepareIndices(const unsigned num_unique_keys, unsigned *d_indices);

//! Calls the kernel that performs the retrieval from the table.
void CallHashRetrieveMultiSorted(
    const unsigned n_queries, const unsigned num_hash_functions,
    const unsigned *d_query_keys, const unsigned table_size,
    const Entry *d_table, const uint2 *d_index_counts,
