...
inline __device__ __host__ unsigned get_key(Entry entry) {
 ...
 */
template <unsigned kNumHashFunctions>
__device__ unsigned retrieve(const unsigned query_key,
...
__global__ void hash_retrieve(const unsigned n_queries, const unsigned *keys_in,
  ...
  unsigned thread_index = threadIdx.x + blockIdx.x * blockDim.x +
                          blockIdx.y * blockDim.x * gridDim.x;
  if (thread_index >= n_queries) return;
...
__device__ unsigned determine_next_location(
    const Functions<kNumHashFunctions> constants, const unsigned table_size,
    const unsigned key, const unsigned previous_location) {
...
__device__ bool insert(const unsigned table_size,
                       ...
                       unsigned *stash_count, Entry entry,
...
__global__ void CuckooHash(const unsigned n_entries, const unsigned *keys,
                           ...
                           unsigned *failures, unsigned *iterations_taken) {
  // Check if this thread has an item and if any previous threads failed.
  unsigned thread_index = threadIdx.x + blockIdx.x * blockDim.x +
                          blockIdx.y * blockDim.x * gridDim.x;
  if (thread_index >= n_entries || *failures) return;
